/**************************************************************************
  * @file       : el_simple_binding_server.c
...
static void reply_status(const el_simple_binding_server_t *p_server, const access_message_rx_t *p_message, el_simple_binding_data_t binding_data)
{
    el_simple_binding_msg_status_t status;
    status.present_binding_data = binding_data;
    access_message_tx_t reply;
    reply.opcode.opcode = EL_SIMPLE_BINDING_OPCODE_STATUS;
    reply.opcode.company_id = EL_SIMPLE_BINDING_COMPANY_ID;
...
{
    el_simple_binding_server_t *p_server = p_args;
    NRF_MESH_ASSERT(p_server->set_cb != NULL);

    el_simple_binding_data_t value = ((el_simple_binding_msg_set_t *)p_message->p_data)->binding_data;
    value = p_server->set_cb(p_server, value);
    reply_status(p_server, p_message, value);
    (void)el_simple_binding_server_status_publish(p_server, value);
}
...
{
    el_simple_binding_server_t * p_server = p_args;
    el_simple_binding_data_t value = ((el_simple_binding_msg_set_unreliable_t*)p_message->p_data)->binding_data;
    value = p_server->set_cb(p_server, value);
    (void)el_simple_binding_server_status_publish(p_server, value);
}
...
{
    {ACCESS_OPCODE_VENDOR(EL_SIMPLE_BINDING_OPCODE_SET,              EL_SIMPLE_BINDING_COMPANY_ID), handle_set_cb},
    {ACCESS_OPCODE_VENDOR(EL_SIMPLE_BINDING_OPCODE_GET,              EL_SIMPLE_BINDING_COMPANY_ID), handle_get_cb},
    {ACCESS_OPCODE_VENDOR(EL_SIMPLE_BINDING_OPCODE_SET_UNRELIABLE,   EL_SIMPLE_BINDING_COMPANY_ID), handle_set_unreliable_cb}
};
...
uint32_t el_simple_binding_server_init(el_simple_binding_server_t * p_server, uint16_t element_index)
{
    ...
    init_params.model_id.model_id = EL_SIMPLE_BINDING_SERVER_MODLE_ID;
...
uint32_t el_simple_binding_server_status_publish(el_simple_binding_server_t *p_server, el_simple_binding_data_t binding_data)
{
    ...
    msg.opcode.opcode = EL_SIMPLE_BINDING_OPCODE_STATUS;
    msg.opcode.company_id = EL_SIMPLE_BINDING_COMPANY_ID;

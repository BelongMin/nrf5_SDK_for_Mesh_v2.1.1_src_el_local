/**************************************************************************
  * @file       : el_simple_color_temperature_client.c
...
{
    el_simple_color_temperature_client_t * p_client = p_args;
    ...
    switch (status)
    {
        ...
        case ACCESS_RELIABLE_TRANSFER_TIMEOUT:
            p_client->status_cb(p_client, EL_SIMPLE_COLOR_TEMPERATURE_STATUS_ERROR_NO_REPLY, NRF_MESH_ADDR_UNASSIGNED);
            break;
        case ACCESS_RELIABLE_TRANSFER_CANCELLED:
            p_client->status_cb(p_client, EL_SIMPLE_COLOR_TEMPERATURE_STATUS_CANCELLED, NRF_MESH_ADDR_UNASSIGNED);
            break;
        default:
...
static bool is_valid_source(const el_simple_color_temperature_client_t * p_client,
...
static uint32_t send_reliable_message(const el_simple_color_temperature_client_t * p_client, el_simple_color_temperature_opcode_t opcode, const uint8_t * p_data, uint16_t length)
{
    ...
    reliable.message.opcode.company_id = EL_SIMPLE_COLOR_TEMPERATURE_COMPANY_ID;
    reliable.message.force_segmented = false;
    reliable.message.transmic_size = NRF_MESH_TRANSMIC_SIZE_DEFAULT;
    reliable.reply_opcode.opcode = EL_SIMPLE_COLOR_TEMPERATURE_OPCODE_STATUS;
    reliable.reply_opcode.company_id = EL_SIMPLE_COLOR_TEMPERATURE_COMPANY_ID;
...
static void handle_status_cb(access_model_handle_t handle, const access_message_rx_t * p_message, void * p_args)
{
    ...
    el_simple_color_temperature_msg_status_t * p_status = (el_simple_color_temperature_msg_status_t *)p_message->p_data;

    if(p_status->present_color_temperature >= EL_SIMPLE_COLOR_TEMPERATURE_VALUE_MIN && p_status->present_color_temperature <= EL_SIMPLE_COLOR_TEMPERATURE_VALUE_MAX)
    {
        el_simple_color_temperature_status_t color_temperature_status = EL_SIMPLE_COLOR_TEMPERATURE_STATUS_NORMAL;
        p_client->status_cb(p_client, color_temperature_status, p_message->meta_data.src.value);
    }
}

static const access_opcode_handler_t m_opcode_handlers[] = 
{
     {{EL_SIMPLE_COLOR_TEMPERATURE_OPCODE_STATUS, EL_SIMPLE_COLOR_TEMPERATURE_COMPANY_ID}, handle_status_cb}
};
...
uint32_t el_simple_color_temperature_client_init(el_simple_color_temperature_client_t * p_client, uint16_t element_index)
{
    ...
    init_params.model_id.model_id = EL_SIMPLE_COLOR_TEMPERATURE_MODEL_CLIENT_ID;
    init_params.model_id.company_id = EL_SIMPLE_COLOR_TEMPERATURE_COMPANY_ID;
...
uint32_t el_simple_color_temperature_client_set(el_simple_color_temperature_client_t * p_client, uint8_t color_temperature)
{
    ...
    p_client->state.data.color_temperature = color_temperature;
    p_client->state.data.tid = m_tid++;

    uint32_t status = send_reliable_message(p_client,
                                            EL_SIMPLE_COLOR_TEMPERATURE_OPCODE_SET,
                                            (const uint8_t *)&p_client->state.data,
                                            sizeof(el_simple_color_temperature_msg_set_t));
...
{
    el_simple_color_temperature_msg_set_unreliable_t set_unreliable;
    set_unreliable.color_temperature = color_temperature;
    set_unreliable.tid = m_tid++;

    access_message_tx_t message;
    message.opcode.opcode = EL_SIMPLE_COLOR_TEMPERATURE_OPCODE_SET_UNRELIABLE;
    message.opcode.company_id = EL_SIMPLE_COLOR_TEMPERATURE_COMPANY_ID;
...
uint32_t el_simple_color_temperature_client_get(el_simple_color_temperature_client_t * p_client)
{
    ...
    uint32_t status = send_reliable_message(p_client,
                                            EL_SIMPLE_COLOR_TEMPERATURE_OPCODE_GET,
                                            NULL,
                                            0);
    if (status == NRF_SUCCESS)
    {
